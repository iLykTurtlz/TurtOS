...
check_cpuid:
	...
	; Copy to ECX as well to compare later on
	mov ecx, eax
	...
	; Copy FLAGS back to EAX (with the flipped bit if CPUID supported)
	pushfd
	pop eax

	; Restore FLAGS from the old version stored in ECX
	; i.e. flipping the ID bit back if it had been flipped
	push ecx
	popfd

	; Compare EAX and ECX.  Equal means the bit was not flipped, CPUID unsupported
	cmp eax, ecx
	je .no_cpuid
	ret
.no_cpuid:
...
.map_p2_table:
	...
	cmp ecx, 512		; if counter == 512, done mapping P2 table
...
enable_paging:
	...
	mov eax, p4_table
	mov cr3, eax   ; WHY THE DOUBLE MOVE?
